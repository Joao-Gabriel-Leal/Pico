...
export default function PicoPage() {
  ...
  async function loadPage() {
    ...
    if (token) {
      requests.push(apiRequest('/api/people', { token }))
    }
    ...
    setEditForm({
      name: picoPayload.item.name,
      primarySportId: picoPayload.item.sport.id,
      description: picoPayload.item.description,
      latitude: String(picoPayload.item.latitude),
      longitude: String(picoPayload.item.longitude),
      statusText: picoPayload.item.statusText,
      conditionLabel: picoPayload.item.conditionLabel,
      coverImageUrl: picoPayload.item.coverImageUrl || '',
    })
  }
  ...
  async function handleEventSubmit(event) {
    ...
    try {
      const payload = await apiRequest(`/api/picos/${slug}/events`, {
        method: 'POST',
        token,
        body: eventForm,
      })
      ...
      setMessage(
        payload.item?.approvalStatus === 'approved'
          ? 'Evento publicado no perfil do pico.'
          : 'Evento enviado para aprovacao do time do pico.',
      )
    } catch (nextError) {
      setError(nextError.message)
    }
  ...
  async function handleDeletePico() {
    ...
    try {
      await apiRequest(`/api/picos/${slug}`, {
        method: 'DELETE',
        token,
      })
      navigate('/mapa')
  ...
  async function handleApprovePico() {
    ...
    try {
      const payload = await apiRequest(`/api/moderation/picos/${slug}/approve`, {
        method: 'POST',
        token,
      })
      setDetail(payload.item)
  ...
  async function handleRejectPico() {
    ...
    try {
      const payload = await apiRequest(`/api/moderation/picos/${slug}/reject`, {
        method: 'POST',
        token,
      })
      setDetail(payload.item)
  ...
  async function handleAddAdmin(event) {
    ...
    try {
      const payload = await apiRequest(`/api/picos/${slug}/admins`, {
        method: 'POST',
        token,
        body: {
          userId: adminUserId,
  ...
  async function handleRemoveAdmin(targetUserId) {
    ...
    try {
      const payload = await apiRequest(`/api/picos/${slug}/admins/${targetUserId}`, {
        method: 'DELETE',
        token,
      })
      setDetail(payload.item)
  ...
  const activeCampaign = useMemo(
    () => detail?.campaigns?.find((item) => item.status === 'active') || null,
    [detail],
  )

  const photoItems = useMemo(
    () => detail?.media?.filter((item) => item.mediaType === 'photo') || [],
    [detail],
  )
    ...
    <section className="page-grid social-page">
      <div className="page-column page-column-main feed-column">
          ...
          <div className="pico-hero-grid">
            <div>
              ...
              <p className="hero-copy">
                {detail.sport.name} - {detail.description}
              </p>
              <p className="muted-text">
                A criacao fica no topo para nao se perder no meio do feed. Quando alguem comum
                sugere algo novo, o time do pico pode aprovar antes de liberar para todo mundo.
              </p>
              ...
              <div className="hero-actions wrap-actions">
                ...
                {user ? (
                  <button className="secondary-button" type="button" onClick={handleVote}>
                    {detail.hasVoted ? 'Tirar voto' : 'Curtir pico'}
                  </button>
                ) : (
                ...
                {detail.permissions?.canPost ? (
                  <button className="primary-button small-link-button" type="button" onClick={() => setShowPostComposer((current) => !current)}>
                    {showPostComposer ? 'Fechar post' : 'Nova publicacao'}
                  </button>
                ) : null}
                {(detail.permissions?.canManageEvents || user?.permissions?.includes('event.submit')) ? (
                  <button className="secondary-button small-link-button" type="button" onClick={() => setShowEventComposer((current) => !current)}>
                    {showEventComposer ? 'Fechar evento' : detail.permissions?.canManageEvents ? 'Novo evento' : 'Sugerir evento'}
                  </button>
                ) : null}
              </div>
            </div>
        ...
        {detail.approvalStatus !== 'approved' ? (
          <div className="side-card moderation-banner">
            <div className="section-title">
              <h2>Status do pico</h2>
              <span>{detail.approvalStatus}</span>
            </div>
            <p className="muted-text">
              {detail.permissions?.canApprovePico
                ? 'Esse pico esta aguardando sua moderacao.'
                : 'Esse pico ainda esta aguardando aprovacao da administracao.'}
            </p>
            {detail.permissions?.canApprovePico ? (
              <div className="inline-actions wrap-actions">
                <button className="primary-button small-link-button" type="button" onClick={handleApprovePico}>
                  Aprovar pico
                </button>
                <button className="ghost-button small-button" type="button" onClick={handleRejectPico}>
                  Rejeitar pico
                </button>
              </div>
            ) : null}
        ...
        {showPostComposer ? (
          <div className="side-card compose-card">
            <div className="section-title">
              ...
              <span>topo do perfil do pico</span>
            </div>
        ...
        {showEventComposer ? (
          <div className="side-card compose-card">
            <div className="section-title">
              <h2>{detail.permissions?.canManageEvents ? 'Novo evento' : 'Sugerir evento'}</h2>
              <span>topo do perfil do pico</span>
            </div>
            <form className="form-card compact-form" onSubmit={handleEventSubmit}>
              ...
              <label>
                ...
                <select
                  ...
                  onChange={(event) => setEventForm((current) => ({ ...current, sportId: Number(event.target.value) }))}
                >
              ...
              <div className="two-column-grid">
                <label>
                  ...
                  <input
                    ...
                    onChange={(event) => setEventForm((current) => ({ ...current, entryFeeCents: Number(event.target.value) }))}
                  />
                </label>
                <label>
                  Premio
                  <input
                    ...
                    onChange={(event) => setEventForm((current) => ({ ...current, prizePoolCents: Number(event.target.value) }))}
                  />
                </label>
              </div>
              ...
              <button className="primary-button full-width">
                {detail.permissions?.canManageEvents ? 'Publicar evento' : 'Enviar para aprovacao'}
              </button>
            </form>
          </div>
        ) : null}
        ...
        <div className="two-card-grid">
          ...
          <div className="side-card">
            ...
            <div className="list-stack compact-list">
              {detail.likedBy.length ? (
                detail.likedBy.map((person) => (
                  <div key={person.id} className="list-item static-item">
                ...
                <p className="muted-text">Ainda nao ha curtidas registradas nesse pico.</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="two-card-grid">
          <div className="side-card">
            <div className="section-title">
              ...
              <span>{detail.admins.length}</span>
            </div>

            <div className="list-stack compact-list">
              {detail.admins.map((person) => (
                <div key={person.id} className="list-item static-item">
                  <div className="user-chip">
                    {person.avatarUrl ? (
                      <MediaAsset className="avatar-circle avatar-mini" src={person.avatarUrl} alt={person.displayName} />
                  ...
                  {detail.permissions?.canManageAdmins ? (
                    <button className="ghost-button small-button" type="button" onClick={() => handleRemoveAdmin(person.id)}>
                      Remover
                    </button>
                  ) : null}
                </div>
        ...
        {detail.permissions?.canEdit ? (
          <div className="side-card">
            <div className="section-title">
              <h2>Gestao do pico</h2>
              <span>permissoes ativas</span>
            </div>

            <div className="inline-actions wrap-actions">
              <button className="secondary-button" type="button" onClick={() => setEditing((current) => !current)}>
                {editing ? 'Fechar edicao' : 'Editar pico'}
              </button>
              {detail.permissions?.canDelete ? (
                <button className="ghost-button" type="button" onClick={handleDeletePico}>
                  Remover pico
                </button>
              ) : null}
            </div>
            ...
            {editing ? (
              <form className="form-card compact-form" onSubmit={handleEditSubmit}>
                ...
                <label>
                  Esporte principal
                  <select
                    value={editForm.primarySportId}
                    onChange={(event) => setEditForm((current) => ({ ...current, primarySportId: Number(event.target.value) }))}
                  >
                    <option value="">Selecione</option>
                    {sports.map((sport) => (
                      <option key={sport.id} value={sport.id}>
                ...
                <div className="two-column-grid">
                  <label>
                    ...
                    <input
                      ...
                      onChange={(event) => setEditForm((current) => ({ ...current, statusText: event.target.value }))}
                    />
                  </label>
                  <label>
                    Selo rapido
                    <input
                      ...
                      onChange={(event) => setEditForm((current) => ({ ...current, conditionLabel: event.target.value }))}
                    />
                  </label>
                </div>
                ...
                <button className="primary-button full-width" disabled={savingEdit}>
        ...
        <div className="list-stack">
          {detail.events.length ? (
                ...
                <div className="post-card-footer">
                  <div className="user-chip">
                    <div className="avatar-circle avatar-mini">
                      {(detail.creator?.displayName || 'P').slice(0, 1).toUpperCase()}
                    </div>
                    <div>
            ...
            <div className="side-card empty-state">
              <p className="muted-text">Esse pico ainda nao tem eventos publicados.</p>
            </div>
          )}
        </div>
        ...
        <div className="list-stack">
          {detail.media.length ? (
            detail.media.map((item) => (
              <SocialPostCard
                key={item.id}
                item={{ ...item, pico: { slug: detail.slug, name: detail.name } }}
                token={token}
                currentUser={user}
      ...
      <aside className="page-column rail-column">
        <div className="side-card sticky-card">
          <div className="section-title">
            <h2>Acoes do pico</h2>
            <span>topo da pagina</span>
          </div>

          <p className="muted-text">
            As criacoes ficam no topo para nao sumirem no meio do feed infinito. Aqui ficam so os
            atalhos e a gestao do pico.
          </p>
          ...
          <div className="list-stack compact-list">
            {detail.permissions?.canPost ? (
              ...
              >
                <div>
                  ...
                  <p>Foto ou video direto no feed do pico</p>
                </div>
                <span>Abrir</span>
              </button>
            ) : null}
            ...
            {(detail.permissions?.canManageEvents || user?.permissions?.includes('event.submit')) ? (
              ...
              >
                <div>
                  <strong>{detail.permissions?.canManageEvents ? 'Novo evento' : 'Sugerir evento'}</strong>
                  <p>Eventos passam por aprovacao quando necessario</p>
                </div>
                <span>Abrir</span>
              </button>
            ) : null}
          ...
          <div className="section-title">
            <h2>Vaquinha</h2>
            <span>{activeCampaign ? 'ativa' : 'nova'}</span>
          </div>
            ...
            <form className="form-card compact-form" onSubmit={handleContribution}>
              <div className="stats-stack">
                <article>
                  ...
                  <strong>{activeCampaign.raisedLabel}</strong>
                </article>
              </div>
            ...
            <form className="form-card compact-form" onSubmit={handleCampaignSubmit}>
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(event) =>
                    setCampaignForm((current) => ({ ...current, goalCents: Number(event.target.value) }))
                  }
                />
              </label>
              ...
              <form className="form-card compact-form" onSubmit={handleAddAdmin}>
                <label>
                  ...
                  <select value={adminUserId} onChange={(event) => setAdminUserId(event.target.value)}>
                    <option value="">Selecione</option>
                    {people
                      .filter((person) => !detail.admins.some((admin) => admin.id === person.id))
                      .map((person) => (
                ...
                <button className="secondary-button full-width" type="submit" disabled={!adminUserId}>
